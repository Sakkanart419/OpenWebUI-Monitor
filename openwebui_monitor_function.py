...
git_url: https://github.com/VariantConst/openwebui_Monitor.git
version: 0.3.6
requirements: httpx
license: MIT
"""
...
TRANSLATIONS = {
    "en": {
        ...
        "group_balance": "Group Balance: ${balance:.4f}",
        "personal_balance": "Personal Balance: ${balance:.4f}",
    ...
    "zh": {
        ...
        "group_balance": "群组余额: ¥{balance:.4f}",
        "personal_balance": "个人余额: ¥{balance:.4f}",
...
class Filter:
    class Valves(BaseModel):
        api_endpoint: str = Field(
            default="", description="openwebui_monitor's base url"
        )
        api_key: str = Field(default="", description="openwebui_monitor's api key")
    ...
    async def inlet(
        ...
        try:
            ...
            if self.outage_map[user_id]:
                balance_key = "insufficient_balance"
                logger.info(
                    self.get_text(
                        balance_key, balance=response_data.get("balance", 0)
                    )
                )
                raise CustomException(
                    self.get_text(
    ...
    async def outlet(
        ...
        try:
            ...
            if self.valves.show_balance:
                ...
                balance_key = "balance"
                if source == "group":
                    balance_key = "group_balance"
                elif source == "personal":
                    balance_key = "personal_balance"
                
                stats_list.append(
                    self.get_text(balance_key, balance=response_data["newBalance"])
                )
